...
def main() -> None:
    ...
    validation_predictions.to_csv(
        OUT_DIR / "lightgbm_validation_predictions.csv", index=False
    )
    validation_summary.to_csv(
        OUT_DIR / "lightgbm_validation_summary.csv", index=False
    )
    locked_predictions.to_csv(
        OUT_DIR / "lightgbm_locked_predictions.csv", index=False
    )
    locked_summary.to_csv(
        OUT_DIR / "lightgbm_locked_summary.csv", index=False
    )
    comparisons.to_csv(
        OUT_DIR / "lightgbm_paired_comparisons.csv", index=False
    )
    ...
    summary = {
        ...
        "locked_rmse_t_ha": float(locked_selected["rmse_t_ha"]),
        "locked_r2": float(locked_selected["r2"]),
        "module_a_delta_rmse_t_ha": float(
            module_a_rmse["delta_left_minus_right"]
        ),
        "module_a_ci95_low": float(module_a_rmse["ci95_low"]),
        "module_a_ci95_high": float(module_a_rmse["ci95_high"]),
        "module_a_pass": bool(module_a_rmse["ci95_high"] < 0.0),
        "module_b_delta_rmse_t_ha": float(
            module_b_rmse["delta_left_minus_right"]
        ),
        "module_b_ci95_low": float(module_b_rmse["ci95_low"]),
        "module_b_ci95_high": float(module_b_rmse["ci95_high"]),
        "module_b_pass": bool(module_b_rmse["ci95_high"] < 0.0),
        "bootstrap_draws": str(BOOTSTRAP_DRAWS.relative_to(ROOT)),
        "bootstrap_draws_sha256": file_sha256(BOOTSTRAP_DRAWS),
    }
    (OUT_DIR / "lightgbm_summary.json").write_text(
