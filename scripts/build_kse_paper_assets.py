...
def write_macros(
    ...
    write_text_lf(OUT / "audit_numbers_kse.tex", text + "\n")
...
def write_module_table(
    ...
    lines = [
        r"\begin{tabular}{@{}cp{0.22\columnwidth}p{0.48\columnwidth}c@{}}",
        r"\toprule",
        r"Mod. & Locked contrast & Estimate & Verdict \\",
        r"\midrule",
    ]
    for row in rows:
        lines.append(" & ".join(row) + r" \\")
    lines += [r"\bottomrule", r"\end{tabular}"]
    write_text_lf(OUT / "table_modules_kse.tex", "\n".join(lines) + "\n")
...
def write_baseline_table(
    ...
    for _, row in baselines.iterrows():
        ...
        if row.config_id == "zero_residual":
            ...
            delta = (
                f"{pair.delta_left_minus_right:.4f} "
    ...
    lines += [r"\bottomrule", r"\end{tabular}"]
    write_text_lf(OUT / "table_baselines_kse.tex", "\n".join(lines) + "\n")
...
def write_synthetic_table(frame: pd.DataFrame) -> None:
    ...
    text = "\n".join(
        [
            r"\begin{minipage}[t]{0.55\textwidth}",
            r"\centering",
            ...
            r"\begin{minipage}[t]{0.42\textwidth}",
            r"\centering",
            r"\begin{tabular}{@{}lrrrr@{}}",
            r"\toprule",
            right,
    ...
    write_text_lf(OUT / "table_synthetic_counts_kse.tex", text + "\n")
...
def main() -> None:
    ...
    write_text_lf(
        OUT / "kse_asset_build_report.json",
